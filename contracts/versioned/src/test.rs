...
fn test_initialize_ok() {
    ...
    client
        .try_initialize(&admin, &1, &0, &0, &s(&env, "v1"))
        .unwrap();
...
fn test_migrate_to_current_fails() {
    ...
    assert_eq!(
        client
            .try_migrate_to_version(&admin, &0)
            .unwrap_err()
            .unwrap(),
...
fn test_migrate_to_out_of_range_fails() {
    ...
    assert_eq!(
        client
            .try_migrate_to_version(&admin, &99)
            .unwrap_err()
            .unwrap(),
...
fn test_non_admin_cannot_migrate() {
    ...
    assert_eq!(
        client
            .try_migrate_to_version(&other, &1)
            .unwrap_err()
            .unwrap(),
        Error::Unauthorized
    );
...
fn test_multiple_migrations_logged() {
    ...
    client
        ...
        .unwrap();
    client
        .try_register_version(&admin, &3, &0, &0, &s(&env, "v3"))
        .unwrap();
    client.try_migrate_to_version(&admin, &1).unwrap();
